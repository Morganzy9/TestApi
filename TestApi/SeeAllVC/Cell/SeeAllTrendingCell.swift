...
final class SeeAllTrendingCell: UICollectionViewCell {
    ...
    private let backGroundDishImage: UIImageView = {
        ...
        backGroundDishImage.contentMode = .scaleToFill
    ...
    private let markOfDishLabel: UILabel = {
        let markOfDishLabel =  UILabel(textColor: .purpleText ?? .white, font: UIFont(name: Theme.Fonts.appFont, size: 20) ?? .systemFont(ofSize: 20))
        markOfDishLabel.layer.shadowColor = UIColor.gray.cgColor
        markOfDishLabel.layer.shadowOffset = CGSize(width: 2, height: 2)
        markOfDishLabel.layer.shadowRadius = 4
        markOfDishLabel.layer.shadowOpacity = 1.5
        return markOfDishLabel
    }()
    
    private let describtionOfDishLabel: UILabel = {
        let describtionOfDishLabel = UILabel(textColor: .purpleText ?? .white, font: UIFont(name: Theme.Fonts.extraBoldFont, size: 20) ?? .systemFont(ofSize: 20))
        describtionOfDishLabel.layer.shadowColor = UIColor.gray.cgColor
        describtionOfDishLabel.layer.shadowOffset = CGSize(width: 2, height: 2)
        describtionOfDishLabel.layer.shadowRadius = 4
        describtionOfDishLabel.layer.shadowOpacity = 1.5
    ...
    private let activityIndicator: UIActivityIndicatorView = {
        ...
        indicator.color = .activeIndicator
        return indicator
    }()
